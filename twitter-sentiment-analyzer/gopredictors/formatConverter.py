...
#with open('data/gop/august/august_full_active.csv', 'rU') as csvFile:
with open('data/gop/march/combined_sample_unique.csv', 'rU') as csvFile:
    reader = csv.DictReader(csvFile)

    for row in reader:
        item = []
        sentiment =  str(row["quote_sentiment"]).lower().replace(' ','').replace('\r','').replace('\n', '')
...
#with open('data/gop/august/august_full_active_form.csv',"wb+") as ofile:
